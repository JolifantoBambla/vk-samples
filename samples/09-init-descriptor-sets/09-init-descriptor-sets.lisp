    ...
    (multiple-value-bind (mvpc size-of-mvpc) (make-mvpc)
      (with-uniform-buffer (uniform-buffer
        ...
        (with-simple-descriptor-set-layout (descriptor-set-layout device)
          (let* ((pool-size (vk:make-descriptor-pool-size
                             :type :uniform-buffer
                             :descriptor-count 1))
                 (descriptor-pool-create-info (vk:make-descriptor-pool-create-info
            ...
            (unwind-protect
                 (let* ((descriptor-set-allocate-info (vk:make-descriptor-set-allocate-info
                   ...
                   (unwind-protect
                        (let* ((descriptor-buffer-info (vk:make-descriptor-buffer-info
                                                        :buffer uniform-buffer
                                                        :offset 0
                                                        :range size-of-mvpc))
                               (write-descriptor-set (vk:make-write-descriptor-set
