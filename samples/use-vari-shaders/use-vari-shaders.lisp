...
(in-package #:vk-samples/use-vari-shaders)

(varjo:define-vari-struct uniform-buffer ()
  (mvpc :mat4))

(defparameter *vertex-stage*
...
(defparameter *compiled-stages*
  (varjo:rolling-translate (list *vertex-stage* *fragment-stage*)))

(defparameter *vertex-shader-code*
  (varjo:glsl-code (first *compiled-stages*)))

(defparameter *fragment-shader-code*
  (varjo:glsl-code (second *compiled-stages*)))

(defun use-vari-shaders (&optional (show-cube-seconds 1) (app-name "use-vari-shaders") (window-width 500) (window-height 500))
  ...
  (with-gfx (instance
    ...
    (with-simple-descriptor-set-layout (descriptor-set-layout
      ...
      (with-simple-pipeline-layout (pipeline-layout
        ...
        (with-compiled-shader-module (vertex-shader-module
                                      device
                                      (shaderc:compile-to-spv *vertex-shader-code*
                                                              :vertex-shader))
          (with-compiled-shader-module (fragment-shader-module
                                        device
                                        (shaderc:compile-to-spv *fragment-shader-code*
                                                                :fragment-shader))
